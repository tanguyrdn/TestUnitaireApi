...
describe('test unitaire', function () {
    ...
    describe('GET result', function () {
        ...
        it('key books should be ‘error fetching books’ ', function (done) {
            const nocky = nock('http://localhost:8080').get('/book').reply(400,{
            ...
            chai
                ...
                .get("/book")
                .end((err, res) => {
                    expect(res.body.message).to.be.equal("error fetching books");
                    done(); // <= Call done to signal callback end
                });
